...
func getCLIPath(config CLIConfig) string {
	...
	path, err := exec.LookPath("xpu-container-cli")
	if err != nil {
		log.Panicln("couldn't find binary xpu-container-cli in", os.Getenv("PATH"), ":", err)
	}
	return path
}
...
func createContainer(containerId string) {
	var err error
	...
	nvidia := container.Nvidia
	if nvidia == nil {
		// Not a XPU container, nothing to do.
		return
	}
...
func poststop(containerId string) {
	var err error
