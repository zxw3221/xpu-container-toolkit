...
func TestAddHookModifier(t *testing.T) {
	...
	testHookPath := filepath.Join(cfg.binPath, "xpu-container-runtime-hook")
			...
			expectedSpec: specs.Spec{
				Hooks: &specs.Hooks{
					...
					CreateContainer: []specs.Hook{
						{
							Path: testHookPath,
							Args: []string{testHookPath, "createContainer", "argv"},
						},
					},
					Poststop: []specs.Hook{
						{
							Path: testHookPath,
							Args: []string{testHookPath, "poststop", "argv"},
						},
					},
				},
			},
		...
		},
		{
			...
			spec: specs.Spec{
				Hooks: &specs.Hooks{
					Prestart: []specs.Hook{
						{
							Path: "xpu-container-runtime-hook",
						},
					},
				},
			},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			m := NewStableRuntimeModifier(logger, argv)
